...
class DigitalAssetGovernanceAgent(BaseAgent):
    ...
    def _run_security_checks(self, url: str, options: dict) -> dict:
        ...
        from src.worker.checks.header_checker import check_headers
        from src.worker.checks.https_checker import check_https_redirect
        from src.worker.checks.library_checker import check_libraries
        from src.worker.checks.reflection_checker import check_reflections
        from src.worker.checks.tls_checker import check_tls
        ...
        if options.get("allow_active", False):
            try:
                from src.worker.zap_client import is_zap_available, zap_baseline_scan
