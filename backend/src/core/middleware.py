...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _get_ttl_seconds(self) -> int:
        ...
        return max(1, seconds_until_reset)
        ...
        try:
            ...
            if current_count > self.max_requests:
                return JSONResponse(
                    status_code=429,
                    content={
                        "detail": f"Rate limit exceeded. Max {self.max_requests} requests per day."
                    },
            ...
            if ttl > 0:
                reset_timestamp = int((datetime.now(UTC) + timedelta(seconds=ttl)).timestamp())
