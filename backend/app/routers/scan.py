...
from app.utils.cache import CacheKeys, cache
from app.utils.errors import active_consent_required_error, invalid_url_error
from app.utils.reports import generate_json_report, generate_pdf_report
from app.utils.verification import domain_is_verified, save_verification_request
...
def create_scan(
    ...
    if not is_verified:
        ...
        unexpired_token = session.exec(
            ...
            .where(DomainVerification.token_expires_at > datetime.now(UTC))
        ...
        if unexpired_token:
            instructions = {
                ...
                "header": {"name": "X-VibeSecure-Verify", "value": "<your-token>"},
            }
        ...
        try:
            ...
            instructions = {
                ...
                "header": {"name": "X-VibeSecure-Verify", "value": token},
            }
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to generate verification token: {str(e)}"
            )

    scan_options: dict[str, Any] | None = getattr(scan, "options", None) or {}
...
def get_scan_report(
    ...
    if cached_report:
        return JSONResponse(
            ...
            headers={
                "Content-Disposition": f'attachment; filename="vibesecure_security_report_{scan_id}.json"'
            },
        )
...
def get_ai_summary(
    ...
    if scan.status != ScanStatus.done:
        raise HTTPException(
            status_code=400, detail=f"Scan is not completed yet. Current status: {scan.status}"
        )
  ...
  "recommendations": ["General security best practice 1", "General security best practice 2"]
    ...
    for attempt in range(max_retries):
        try:
            ...
            is_retryable = any(
                code in error_msg
                for code in ["503", "500", "502", "504", "429", "timeout", "unavailable"]
            )
            ...
            raise HTTPException(
                ...
                detail=f"AI summary generation failed: {str(e)}. Please try again later."
                if is_retryable
                else f"AI summary generation failed: {str(e)}",
            )
...
def generate_vercel_config(headers: dict, url: str) -> str:
    ...
    if headers["csp"]:
        header_configs.append(
            "      { \"key\": \"Content-Security-Policy\", \"value\": \"default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self'; frame-ancestors 'none'\" }"
        )
    ...
    headers_json = (
        ",\n".join(header_configs)
        if header_configs
        else '      { "key": "X-Content-Type-Options", "value": "nosniff" }'
    )
