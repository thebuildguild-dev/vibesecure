...
def save_verification_request(
    session: Session, domain: str, user_email: str
    ...
    if len(domain_requests_count) >= settings.domain_verification_max_requests_per_domain_per_day:
        ...
        raise rate_limit_error(
            f"domain {domain}", settings.domain_verification_max_requests_per_domain_per_day
        )
    ...
    if len(user_requests_count) >= settings.domain_verification_max_requests_per_user_per_day:
        ...
        raise rate_limit_error("user", settings.domain_verification_max_requests_per_user_per_day)
...
def check_token_on_site(session: Session, verification_id: str) -> dict:
    ...
    if not verification:
        ...
        return {"verified": False, "method": None, "details": "Verification record not found"}
            ...
            try:
                ...
                if method == "file":
                    ...
                    if "vibesecure-verify" in content:
                        ...
                        match = re.search(
                            r'vibesecure-verify["\']?\s*[=:]\s*["\']?([a-f0-9-]{36})',
                            content,
                            re.IGNORECASE,
                        )
                        if match:
                ...
                if token_from_site:
                    ...
                    if matched_record:
                        ...
                        matched_record.verified_at = datetime.now(UTC)
                ...
                audit_entry = DomainVerificationAudit(
                    verification_id=verification_id, action="failed_check", details=error_msg
                )
                session.add(audit_entry)
                continue
    ...
    return {"verified": False, "method": None, "details": details_msg}


def domain_is_verified(session: Session, domain: str, user_email: str) -> DomainVerification | None:
    now = datetime.now(UTC)
