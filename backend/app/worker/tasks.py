...
from app.worker.celery_app import BACKEND_URL, DATABASE_URL, celery_app
...
from app.worker.scanner import ScanResult, scan_url
...
def process_scan(self, scan_id: int, url: str):
    ...
    with Session(worker_engine) as session:
        ...
        if allow_active:
            ...
            logger.warning(
                f"[Scan {scan_id}] Active scan consent GRANTED for {verification.domain} (verified_at={consent.verified_at})"
            )
        ...
        scan.result = json.dumps(
            {
                ...
                "verification_timestamp": verification.verified_at.isoformat()
        ...
        if "localhost" in url or "127.0.0.1" in url:
            ...
            logger.info(
                f"[Scan {scan_id}] Adjusted URL for Docker environment: {url} -> {target_url}"
            )
    ...
    try:
        ...
        all_findings = (
            ...
            + library_findings
            + reflection_findings
            + (result.findings if result.findings else [])
        )
                ...
                if is_zap_available():
                    ...
                    for zap_finding in zap_findings:
                        key = (
                            ...
                            zap_finding.get("path", zap_finding.get("url", "")),
                        )
        ...
        logger.info(
            f"[Scan {scan_id}] Completed: {len(all_findings)} findings in {result.duration_seconds:.2f}s"
        )
        ...
        if self.request.retries < self.max_retries:
            logger.info(
                f"[Scan {scan_id}] Retrying ({self.request.retries + 1}/{self.max_retries})..."
            )
